...
class TestMainPage:
    @allure.title('Проверка редиректа по клику на "Конструктор"')
    ...
    @allure.title('Проверка редиректа по клику на "Лента заказов"')
    ...
    @allure.title('Проверка закрытия окна "Детали ингредиента" кликом по Х')
    ...
    def test_changing_counter_for_ingredients_in_order_success(self, driver):
        ...
        assert main_page.get_count_of_ingredients() != 0
