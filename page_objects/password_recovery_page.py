...
class PasswdRecoveryPage(BasePage):
    ...
    @allure.step('Проверить, что значение поля password не отображается (скрыто)')
    ...
    @allure.step('Проверка появления активной рамки после клика по иконке глаза')
    def check_displaying_active_border(self):
        return self.find_element_with_wait(PasswordRecoveryLocators.input_status_is_active)